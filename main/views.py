...
def index(request):
    ...
    if data:
        ...
        if any([data["is_open"], data["location"], data.get("is_for_disabled")]):
            place_type = ParkingPlaceType.objects.filter(
                is_open=data["is_open"] or None,
                location=data["location"] or None,
                is_for_disabled=True if data.get("is_for_disabled") else False,
            ).first()
        elif preferences.is_exist():
            place_type = ParkingPlaceType.objects.filter(
                is_open=preferences.is_open,
                location=preferences.location,
                is_for_disabled=preferences.is_for_disabled
            ).first()
